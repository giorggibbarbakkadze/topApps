...
class MainActivity : AppCompatActivity() {
    ...
    private var currentURL: String = "None"
        ...
        when (item.itemId) {
            ...
            R.id.mnu10, R.id.mnu25 -> {
                if (!item.isChecked) {
                    ...
                    Log.d(
                        TAG,
                        "onOptionsItemSelected: ${item.title} setting feedLimits to $feedLimit"
                    )
                } else {
                    Log.d(TAG, "onOptionsItemSelected: ${item.title} setting feedLimit unchanged")
                }
            }
            R.id.mnuRefresh -> currentURL = "None"
    ...
    companion object {
        private class DownloadData(context: Context, listView: ListView) :
            ...
            val feedName by lazy<TextView> { (propContext as Activity).findViewById(R.id.feedName) }
            ...
            override fun onPostExecute(result: String) {
                ...
                val appParse = ParseApplications()
                appParse.parse(result)
                feedName.text = appParse.feedName
                val feedAdapter =
                    FeedAdapter(propContext, R.layout.list_record, appParse.applications)
